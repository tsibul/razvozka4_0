...
async function buildRowForSingle(element) {
    const newRow = document.createElement('tr');
    newRow.classList.add('edit-modal');
    newRow.dataset.id = element['id'];
    //date
    let newCell = document.createElement('td');
    let cellContent = document.createTextNode(dateRusLong(element['date']));
    newCell.appendChild(cellContent);
    newRow.appendChild(newCell);
    // driver icon
    newCell = document.createElement('td');
    ...
    // customer_name
    newCell = document.createElement('td');
    newCell.innerHTML = '<strong>' + element['customer_name'] + '</strong>';
    newRow.appendChild(newCell);
    // address
    newCell = document.createElement('td');
    newCell.innerHTML = element['address'];
    newRow.appendChild(newCell);
    //contact
    newCell = document.createElement('td');
    newCell.innerHTML = element['contact'];
    newRow.appendChild(newCell);
    // to_do
    newCell = document.createElement('td');
    let toDoString = '';
    if (element['to_do_take']) {
    ...
    if (element['return_from']) {
        newCell.classList.add('title-icon-big');
        newCell.setAttribute('onmouseenter', 'plannedReturnsList(' + element['id'] + ', this);');
        button = document.createElement('button');
        button.type = 'button';
        button.classList.add('btn-submit', 'btn-bg');
        button.innerHTML = diggingIcon;
    ...
    if (element['customer_id'] != null) {
        ...
        if (customer['subcontractor'] && element['to_do_deliver'] != '') {
            ...
            button.type = 'button';
            button.classList.add('btn-delete', 'btn-bg');
            button.setAttribute('onclick', 'razvozkaDeliverTo(this, ' + element['id'] + ');');
            button.innerHTML = deliverToIcon;
            if (element['deliver_to']) {
                button.classList.add('delete-neg');
            }
            newCell.appendChild(button);
        }
    ...
    button.type = 'button';
    button.classList.add('btn-delete', 'btn-bg');
    button.setAttribute('onclick', 'razvozkaDelete(this, ' + element['id'] + ');');
    button.innerHTML = deleteIcon;
    newCell.appendChild(button);
    newRow.appendChild(newCell);
    return newRow;
